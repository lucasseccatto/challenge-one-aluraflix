...
const filmesApi =
  "https://my-json-server.typicode.com/tsilfidis/aluraflix-api/filmes";
const categoriasApi =
  "https://my-json-server.typicode.com/tsilfidis/aluraflix-api/categorias";
...
export default function FilmesProvider({ children }) {
  ...
  useEffect(() => {
    axios.get(filmesApi).then((response) => {
...
export function useFilmesContext() {
  ...
  function updateGame(filme) {
    axios
      ...
      .then(() => {
        setFilmes(
          filmes.map((thisfilme) =>
            thisfilme.id === filme.id ? filme : thisfilme
          )
        );
